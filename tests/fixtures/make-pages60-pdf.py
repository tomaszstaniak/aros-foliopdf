...
"""Write pages60.pdf: a sixty-page fixture made for this project (no
...
A4 portrait; the bar hue advances by 6 degrees per page. Sixty A4 pages at
view width exceed 32767 px, the limit of MUI layout sizes.
...
pages = []
for k in range(60):
    r, g, b = colorsys.hsv_to_rgb(k / 60.0, 0.9, 0.85)
    pages.append((SIZES[0], f"{r:.2f} {g:.2f} {b:.2f}"))
n = len(pages)
...
for k in range(1, size):
    out += b"%010d 00000 n \n" % offsets[k]
out += b"trailer\n<< /Size %d /Root 1 0 R >>\nstartxref\n%d\n%%%%EOF\n" % (size, xref)
dest = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).with_name("pages60.pdf")
